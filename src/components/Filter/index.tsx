function Filter({ onColorFilterChange, products }: FilterProps) {
    const availableColors = Array.from(new Set(products.map(product => product.colour)));

    return (
        <div className="mb-4">
            <label htmlFor="colorFilter" className="block text-gray-700 text-sm font-bold mb-2">
                Filter by Color:
            </label>
            <select
                id="colorFilter"
